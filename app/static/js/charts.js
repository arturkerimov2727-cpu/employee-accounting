...
window.AttendanceCharts = (() => {
  ...
  function drawAttendanceCharts(hoursCanvas, timeCanvas, days, hours, events) {
    return [
      ...
      makeChart(timeCanvas, "line", days.map((day) => label(day)), [{ label: "Приход", data: days.map((day) => averageEventMinutes(events, day, "IN")), borderColor: cssVar("--chart-green", "#18a957"), backgroundColor: cssVar("--chart-green-soft", "rgba(24, 169, 87, .18)"), tension: .35 }, { label: "Уход", data: days.map((day) => averageEventMinutes(events, day, "OUT")), borderColor: cssVar("--chart-orange", "#ef6f21"), backgroundColor: cssVar("--chart-orange-soft", "rgba(239, 111, 33, .18)"), tension: .35 }], { timeAxis: true })
    ].filter(Boolean);
  }
