...
async def mini_payload(request: Request, include_history=True):
    ...
    async with pool.acquire() as connection:
        ...
        history = await attendance_days(connection, employee["id"], history_start, today, settings.get("timezone"), schedule) if include_history else []
        day = history[-1] if history else (await attendance_days(connection, employee["id"], today, today, settings.get("timezone"), schedule))[0]
    return {
